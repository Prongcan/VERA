"""
Qwen3-VL Inference Script with Attention Head Masking (using VERA API)
...
# --- Attention Head Masking 配置 ---
# 格式: {(layer, head), ...}
# 示例：Mask 掉特定层的特定注意力头
MASK_CONFIG = {(29, 24), (11, 21), (8, 24), (26, 26), (13, 24)}
...
def main():
    ...
    # --- 2. 初始化引擎 (使用新的 VERA API - Masked 版本) ---
    print(f"Initializing VERA masked model from {args.model_path}...")
    print(f"Mask configuration: {MASK_CONFIG}")
    engine = models.initialize(
        model_path=args.model_path,
        model_type="qwen-img-masked"  # 使用支持 masking 的版本
    )
    ...
    for paper in tqdm(papers[:args.limit]):
        ...
        for qa in paper.get("qas", []):
            ...
            # --- 5. 运行推理 (使用新的 VERA API - 带 masking) ---
            ...
            try:
                res = engine.run(
                    ...
                    is_mask_heads=True,  # 启用 attention head masking
                    heads_positions=MASK_CONFIG  # 指定要 mask 的头
                )
