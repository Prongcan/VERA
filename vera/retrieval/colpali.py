...
def colpali(
    ...
    for root, dirs, files in os.walk(data_path):
        if "result" in root:
            continue
        if "input_tokens.json" in files and "word_mapping.json" in files:
            folders.append(root)
    ...
    for folder_path in tqdm(folders, desc="Processing with ColPali"):
        try:
            ...
            word_mapping_path = os.path.join(folder_path, "word_mapping.json")
            image_path = os.path.join(folder_path, "merged.png")
            ...
            for crop_img, start_x, start_y, crop_height in crops:
                ...
                # Get query embedding
                batch_queries = processor.process_queries([question]).to(device)
                with torch.no_grad():
